...
function hideLoader() {
  if (!loadingScreen) return; // `.loading-screen` 요소가 없으면 함수 종료
...
document.addEventListener("DOMContentLoaded", () => {
 ...
 // career
  ...
  ScrollTrigger.create({
    ...
    scrub: true,
    onUpdate: (self) => {
      const el = document.querySelector(".black-fill");
  ...
  ScrollTrigger.create({
    trigger: ".comp-career",
    start: "top top",
    end: "bottom center", // 기존 black-fill 전환 시점
    scrub: true,
    onUpdate: (self) => {
      const black = document.querySelector(".black-fill");
      const skillBox = document.querySelector(".skill-box");
      if (self.progress > 0.5) {
        black?.classList.add("full-black");
      } else {
        black?.classList.remove("full-black");
      }

      // 여기에서 skill-box 활성화 시점도 같이 체크
      if (black?.classList.contains("full-black")) {
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      startSkillDropOnce();  // 최초 1회만
      resumeSkillDrop();     // 다시 재생
    },
    onLeave: () => {
      pauseSkillDrop();      // 일시정지
    },
...
if (compMindset) {
  const slides = gsap.utils.toArray('.comp-mindset > .mindset-slide');
  const totalWidth = compMindset.scrollWidth;
...
function dropNextSkill() {
  ...
  el.innerHTML = `<img src="../2025pp/assets/image/icons/${skill.icon}" alt="${skill.name}" /><span>${skill.name}</span>`;
